...
$(VENDORED_DIRS):
	go get -d $(subst $(VENDOR)/,,$@) && mkdir -p $(@D) && \
...
GOGET_PKGS := $(shell comm -13 \
  ...
  sed -e 's@'"$(IMPORT_PATH)/vendor"/'@@' \
      ...
      -e '/^C$$/d' \
      -e '/^\s*$$/d')
GOGET_PKGS := $(filter-out $(VENDORED_PKGS),$(GOGET_PKGS))
GOGET := $(addprefix $(GOPATH)/src/,$(GOGET_PKGS))
$(GOGET):
	go get -d $(subst $(GOPATH)/src/,,$@)
goget: $(GOGET)

# the GODEPS var can be used to depend on vendored packages as
# well as the ones retrieved via "go get"
GODEPS := $(GOGET) $(VENDOR)
